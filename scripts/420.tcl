...
bind pub - !toke do420
proc do420 { nick uhost handle channel arg } {
    putlog "420.tcl: $nick!$uhost ($handle) requested a toke."
    global xmsg
   
    set toke [lindex $xmsg [expr {int(rand()*[llength $xmsg])}]]

    # Send the message to the channel. -ze
    putquick "PRIVMSG $channel :$toke"
}
